...
def load_documents(data_dir: Path) -> List[Document]:
    ...
    for file_path in file_paths:
        try:
            ...
            # Add metadata for IT governance context
            for doc in docs:
                doc.metadata["subdirectory"] = file_path.parent.name
                doc.metadata["file_type"] = suffix
                doc.metadata["document_category"] = "it_governance"
                doc.metadata["source_system"] = "berenberg_it_governance"
...
def main():
    ...
    # Source document folder for IT governance data
    data_dir = Path(r"W:\T1368-WAM AI\03 Projects\02 AI Projects\01 Berenberg AI Hub\16 IT Governance data\no label")
    ...
    chroma_dir = output_dir / "it_governance_chroma_db"
    chroma_dir.mkdir(parents=True, exist_ok=True)
    
    print("=== Creating IT Governance ChromaDB ===")
    ...
    # Load documents
    print("Loading IT governance documents...")
    ...
    vector_store = Chroma.from_documents(
        ...
        collection_name="berenberg_it_governance",
        persist_directory=str(chroma_dir)
    )
    
    print("✅ IT Governance ChromaDB created successfully!")
    print(f"   Location: {chroma_dir}")
    print(f"   Total documents: {len(documents)}")
    print(f"   Total chunks: {len(chunks)}")
    print(f"   Collection name: berenberg_it_governance")
    print()
    
    print("=== Summary ===")
    print("✅ IT Governance ChromaDB creation completed!")
    print(f"📁 Database location: {chroma_dir}")
    print(f"📊 Collection: berenberg_it_governance")

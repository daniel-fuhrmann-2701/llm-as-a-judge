...
def export_evaluation_results_to_excel(evaluation_results: List[Dict[str, Any]], 
    ...
    for result in evaluation_results:
        ...
        # Optional fallback metadata columns
        if "fallback_used" in result:
            row_data["Fallback_Used"] = result.get("fallback_used")
        if "fallback_merge_applied" in result:
            row_data["Fallback_Merge_Applied"] = result.get("fallback_merge_applied")
        if "original_internal_answer_length" in result:
            row_data["Original_Internal_Answer_Length"] = result.get("original_internal_answer_length")
        if "web_fallback_length" in result:
            row_data["Web_Fallback_Length"] = result.get("web_fallback_length")
